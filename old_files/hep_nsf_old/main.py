...
def build_parser() -> argparse.ArgumentParser:
    ...
    mdl.add_argument("--model", choices=["s2", "r3", "r2"],
                     default="s2",
...
def _load_model_from_checkpoint(checkpoint_path, args, device):
    """Read architecture from checkpoint metadata, build model, load weights."""
    meta = torch.load(checkpoint_path, map_location=device)

    # Read saved architecture if present, else fall back to CLI args
    model_type  = meta.get("model_type",  args.model)
...
def run_sample(args: argparse.Namespace) -> None:
    ...
    model, model_type = _load_model_from_checkpoint(args.checkpoint, args, device)
    model.eval()
