    ...
    def forward(self, x, inverse=False):
        ...
        if not inverse:
            ...
            for k in reversed(range(self.num_splines)):
                ...
                # Forward was: cos_theta' = rqs(cos_theta, free_k)
                #              phi'       = rqs_circ(phi, MLP_k(cos_theta'))
                #
                # Inverse must be:
                # Step 1: phi  = rqs_circ_inv(phi', MLP_k(cos_theta))
                #                             using CURRENT (transformed) cos_theta
                # Step 2: cos_theta = rqs_inv(cos_theta', free_k)

                # Step 1: invert phi using CURRENT (still-transformed) cos_theta
                w, h, d = self._phi_whd(k, cos_theta.unsqueeze(1))
                phi, _ = rqs_circular(
                    phi, w, h, d,
                    inverse=True,
