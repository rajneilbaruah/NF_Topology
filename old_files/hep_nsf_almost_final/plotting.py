...
def plot_radius_distribution(samples_cart,
    ...
    if r_max - r_min < 1e-6:
        # All values identical — draw a vertical line instead of a histogram
        ...
        try:
            ax.hist(radii, bins=n_bins, color=MODEL_COLOR, alpha=0.75,
                    histtype="stepfilled", edgecolor=MODEL_COLOR, lw=1.5)
        except ValueError:
            # Range too narrow for n_bins — fall back to automatic binning
            try:
                ax.hist(radii, bins="auto", color=MODEL_COLOR, alpha=0.75,
                        histtype="stepfilled", edgecolor=MODEL_COLOR, lw=1.5)
            except ValueError:
                ax.axvline(np.mean(radii), color=MODEL_COLOR, lw=2,
                           label=f"mean = {np.mean(radii):.2f} GeV")
